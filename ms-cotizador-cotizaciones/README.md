...
mysql -u root -p
CREATE DATABASE cotizador_cotizaciones_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'cotizador_cotizaciones_user'@'localhost' IDENTIFIED BY 'cotizador_cotizaciones_pass';
GRANT ALL PRIVILEGES ON cotizador_cotizaciones_db.* TO 'cotizador_cotizaciones_user'@'localhost';

# 2. Ejecutar scripts de base de datos
mysql -u cotizador_cotizaciones_user -p cotizador_cotizaciones_db < sql/ddl.sql
mysql -u cotizador_cotizaciones_user -p cotizador_cotizaciones_db < sql/dml.sql
...
|----------|-----|--------------|
| **API REST (dev)** | http://localhost:8083/api/v1 | Basic Auth (admin/admin123) |
| **Swagger UI (dev)** | http://localhost:8083/api/v1/swagger-ui/index.html | Basic Auth |
| **Health Check (dev)** | http://localhost:8083/api/v1/actuator/health | - |
| **Base de Datos (dev)** | localhost:3306/cotizador_cotizaciones_db | cotizador_cotizaciones_user / cotizador_cotizaciones_pass |
...
- **Swagger UI (dev)**: http://localhost:8083/api/v1/swagger-ui/index.html
- **OpenAPI JSON (dev)**: http://localhost:8083/api/v1/v3/api-docs
- **Health Check (dev)**: http://localhost:8083/api/v1/actuator/health
...
DB_USERNAME=cotizador_cotizaciones_user    # Usuario de base de datos
DB_PASSWORD=cotizador_cotizaciones_pass    # Contraseña de base de datos
...
# Aplicación
SERVER_PORT=8083                    # Puerto del servidor (dev)
...
spring:
  datasource:
    url: jdbc:mysql://${DB_HOST:localhost}:3306/cotizador_cotizaciones_db
    username: ${DB_USERNAME:cotizador_cotizaciones_user}
    password: ${DB_PASSWORD:cotizador_cotizaciones_pass}
...
USER spring
WORKDIR /app
COPY --from=builder /app/target/*.jar /app/cotizaciones.jar
EXPOSE 8083
ENV SPRING_PROFILES_ACTIVE=docker
ENTRYPOINT ["sh", "-c", "exec java ${JAVA_OPTS} -jar cotizaciones.jar"]
```
...
```bash
# Construir imagen
docker build -t ms-cotizador-cotizaciones ./ms-cotizador-cotizaciones

# Ejecutar contenedor (dev)
docker run -p 8083:8083 \
  -e DB_HOST=localhost \
  -e DB_USERNAME=cotizador_cotizaciones_user \
  -e DB_PASSWORD=cotizador_cotizaciones_pass \
...
docker logs ms-cotizador-cotizaciones

# Verificar conexión a base de datos
mysql -h localhost -u cotizador_cotizaciones_user -p cotizador_cotizaciones_db
