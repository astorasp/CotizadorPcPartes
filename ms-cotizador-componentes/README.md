...
mysql -u root -p
CREATE DATABASE cotizador_componentes_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
mysql -u componentes_user -p cotizador_componentes_db < sql/ddl.sql
mysql -u componentes_user -p cotizador_componentes_db < sql/dml.sql
...
|----------|-----|--------------|
| **API REST (dev)** | http://localhost:8082/api/v1 | Basic Auth (admin/admin123) |
| **Swagger UI (dev)** | http://localhost:8082/api/v1/swagger-ui/index.html | Basic Auth |
| **Health Check (dev)** | http://localhost:8082/api/v1/actuator/health | - |
| **Base de Datos (dev)** | localhost:3306/cotizador_componentes_db | componentes_user / componentes_pass |
...
GET    /api/v1/componentes/{id}                     # Obtener por ID
PUT    /api/v1/componentes/{id}                     # Actualizar componente
DELETE /api/v1/componentes/{id}                     # Eliminar componente
GET    /api/v1/componentes/tipo/{tipo}              # Listar por tipo (MONITOR|DISCO_DURO|TARJETA_VIDEO|PC)
GET    /api/v1/componentes/{id}/existe              # Verificar existencia
GET    /api/v1/componentes/con-pcs                  # Listar todos incluyendo PCs
```
...
GET    /api/v1/pcs/{id}                              # Obtener PC por ID
PUT    /api/v1/pcs/{id}                              # Actualizar PC
DELETE /api/v1/pcs/{id}                              # Eliminar PC
POST   /api/v1/pcs/{id}/componentes                  # Agregar componente a PC
DELETE /api/v1/pcs/{id}/componentes/{componenteId}   # Remover componente
GET    /api/v1/pcs/{id}/componentes                  # Listar componentes de PC
```

<!-- Endpoints de otros microservicios (cotizaciones, pedidos, proveedores) se documentan en sus respectivos repos -->
...
- **Swagger UI (dev)**: http://localhost:8082/api/v1/swagger-ui/index.html
- **OpenAPI JSON (dev)**: http://localhost:8082/api/v1/v3/api-docs
- **Health Check (dev)**: http://localhost:8082/api/v1/actuator/health
...
- **Spring Security** - Autenticación y autorización
  - Perfil `default` (dev): Basic Auth habilitado
  - Perfil `docker`: JWT vía ms-seguridad (Basic deshabilitado)
...
DB_USERNAME=componentes_user        # Usuario de base de datos
DB_PASSWORD=componentes_pass        # Contraseña de base de datos

# Seguridad
SECURITY_USERNAME=admin             # Usuario API básica (dev)
SECURITY_PASSWORD=admin123          # Contraseña API básica (dev)

# Aplicación
SERVER_PORT=8082                    # Puerto del servidor (dev)
...
spring:
  datasource:
    url: jdbc:mysql://${DB_HOST:localhost}:3306/cotizador_componentes_db
...
```dockerfile
# Dockerfile (multi-stage, ver archivo Dockerfile en este módulo)
...
```bash
# Construir imagen
docker build -t ms-cotizador-componentes ./ms-cotizador-componentes

# Ejecutar contenedor (dev)
docker run -p 8082:8082 \
  -e DB_HOST=localhost \
  -e DB_USERNAME=componentes_user \
  -e DB_PASSWORD=componentes_pass \
...
curl http://localhost:8082/api/v1/actuator/info

# Métricas
curl http://localhost:8082/api/v1/actuator/metrics
```
...
docker logs ms-cotizador-componentes

# Verificar conexión a base de datos
mysql -h localhost -u componentes_user -p cotizador_componentes_db

# Verificar endpoints disponibles
curl -u admin:admin123 http://localhost:8082/api/v1/componentes
```
